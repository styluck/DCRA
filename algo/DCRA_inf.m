function [xsol, outs] = DCRA_inf(A, b, V, OPTIONS)
%% ***************************************************************
% filename: QAfactor
...
if isfield(OPTIONS,'maxiter');   submxitr = OPTIONS.submxitr;  else; submxitr =30;  end
...
% get outputs
outs.rho = rho;
outs.obj_list = obj_list;

xsol  = d1*Q1(2:end);

f_sol = obj_func_inf(xsol, A, b);
f_sol2 = obj_func_inf(-xsol, A, b);
if f_sol > f_sol2
    outs.fobj = f_sol2;
    xsol = -xsol;
else
...
z = prox(x, del);
h = norm(z, inf) + .5*(norm(x - z,'fro')^2)./del;
